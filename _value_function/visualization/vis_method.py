...
with open(f'{fpath.resolve()}/{filename}', 'rb') as file:
    ...
    if calc_novf:
        initializations, \
        pregrasp_poses_vf, planned_pregrasp_poses, turn_poses_vf, turn_trajectories_vf, \
        pregrasp_poses_novf, planned_pregrasp_poses_novf, turn_poses_novf, turn_trajectories_novf = zip(*tuples)
    else:
        initializations, pregrasp_poses_vf, planned_pregrasp_poses, turn_poses_vf, turn_trajectories_vf = zip(*tuples)
...
if __name__ == "__main__":
    ...
    for i in range(int(len(initializations)/n_repeat)):
        ...
        for j in range(n_repeat):
            ...
            costs_vf.append(vf)
            if calc_novf:
                novf, _ = calculate_turn_cost(pregrasp_poses_novf[current_index].numpy(), turn_poses_novf[current_index])
    ...
    planned_predicted_costs_vf = []
    planned_predicted_stds_vf = []
    ...
    for i in range(len(pregrasp_poses_vf)//n_repeat):
        ...
        for j in range(n_repeat):
            ...
            planned_vf = planned_pregrasp_poses[i*n_repeat+j]
            planned_vf = convert_full_to_partial_config(planned_vf.reshape(1,20))
            planned_vf_norm = (planned_vf - poses_mean) / poses_std
            planned_vf_norm = planned_vf_norm.float()
            ensemble_vf = query_ensemble(planned_vf_norm, models)
            prediction_vf = ensemble_vf.mean(dim=0)
            std_vf_pred = ensemble_vf.std(dim=0)
            prediction_vf = prediction_vf * cost_std + cost_mean
            std_vf_pred = std_vf_pred * cost_std
            planned_pred_mean.append(prediction_vf.detach().numpy())
            planned_pred_std.append(std_vf_pred.detach().numpy())

        predicted_costs_vf.append(np.mean(np.array(pred_mean)))
        predicted_stds_vf.append(np.mean(np.array(pred_std)))
        
        planned_predicted_costs_vf.append(np.mean(np.array(planned_pred_mean)))
        planned_predicted_stds_vf.append(np.mean(np.array(planned_pred_std)))
    ...
    if vis:
        ...
        for i in range(len(initializations)//n_repeat):
            ...
            print("plan")
            env.reset(planned_pregrasp_poses[i*n_repeat].reshape(1,20))
            time.sleep(initialization_delay)

            print("execution")
            env.reset(pregrasp_poses_vf[i*n_repeat].reshape(1,20).float())
            ...
            # print("initialization, novf")
            # env.reset(initializations[i*n_repeat].reshape(1,20))
            # time.sleep(initialization_delay)

            # print("execution, novf")
            # env.reset(pregrasp_poses_novf[i*n_repeat].reshape(1,20))
            # time.sleep(big_delay)

            # print("after turn, novf")
            # env.reset(turn_poses_novf[i*n_repeat].reshape(1,20))
            # time.sleep(big_delay)

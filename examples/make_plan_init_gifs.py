...
def add_text_to_imgs(imgs, labels):
    ...
    for i, img in enumerate(imgs):
        ...
        draw.text((20, 20), labels[i], (0, 0, 0), font_size=30, align='center')
        # Show image
...
for trial_num in range(0, config['num_trials']):
    ...
    isaac_imgs = [fpath / img for img in sorted(os.listdir(fpath)) if img[-3:] == 'png']
    ...
    # for sample in tqdm(range(1)):
        ...
        for iter, dir_n in (c_imgs):
            ...
            try:
                ...
                labels_planned_inits += [dir_n.split('/')[-1]] * len(gif_imgs_planned_inits[-1])
            except:
                pass
        ...
        if len(gif_imgs_inits) > 0:
            imageio.mimsave(f'{fpath}/init_{sample}.gif', gif_imgs_inits, loop=0)
        if len(gif_imgs_plans) > 0:
            imageio.mimsave(f'{fpath}/plan_{sample}.gif', gif_imgs_plans, loop=0)
        if len(gif_imgs_planned_inits) > 0:
            imageio.mimsave(f'{fpath}/planned_init_{sample}.gif', gif_imgs_planned_inits, loop=0)

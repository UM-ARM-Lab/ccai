...
def plot_long_horizon(test_model, loader, config):
    fpath = f'{CCAI_PATH}/data/training/allegro_screwdriver/{config["model_name"]}_{config["model_type"]}/eta_05'
    pathlib.Path.mkdir(pathlib.Path(fpath), parents=True, exist_ok=True)
    N = 16
...
def generate_simulated_data(model, loader, config):
    ...
    loader.dataset.update_masks(p=1.0) # no masking
    ...
    for trajectories, traj_class, _ in train_loader:
        ...
        start = (trajectories[:, 0, :15] * train_dataset.std[:15].to(device=trajectories.device) +
                 train_dataset.mean[:15].to(device=trajectories.device))
    ...
    np.savez(f'{fpath}/simulated_trajectories.npz', trajectories=simulated_trajectories, contact=simulated_class)
...
def train_classifier(model, train_loader, config):
    ...
    for epoch in pbar:
        ...
        for real_traj, real_class, real_masks, fake_traj, fake_class, fake_masks in train_loader:
            ...
            loss = model.classifier_loss(trajectories, mask=masks, label=labels, context=context)
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(model.parameters(), 1)
            # for param in trajectory_sampler.parameters():
            #    print(param.grad)
...
if __name__ == "__main__":
    ...
    #generate_simulated_data(model, train_loader, config)
    plot_long_horizon(model, train_loader, config)

    fpath = f'{CCAI_PATH}/data/training/allegro_screwdriver/{config["model_name"]}_{config["model_type"]}/simulated_dataset/simulated_trajectories.npz'

    # fake_dataset = FakeDataset(fpath)
    # classifier_dataset = RealAndFakeDataset(fake_dataset, train_dataset)

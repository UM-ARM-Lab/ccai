...
def parse_args():
    ...
    parser.add_argument('--method', type=str, default='csvgd')
...
if __name__ == "__main__":
    ...
    # load data to start
    data = np.load(f'{args.exp_folder}/{args.method}/trial_1/trajectory.npz')
    actual_trajectory = data['x'][:-1]
    planned_trajectories = data['traj']
    #sdf_grid = data['sdf_grid']

    # how many trials are there?
    import pathlib

    path = pathlib.Path(f'{args.exp_folder}/{args.method}')
    files = list(path.rglob('trajectory.npz'))
    ...
    ve = RvizAnimationController(n_time_steps=num_trials, ns='env')
    vt = RvizAnimationController(n_time_steps=actual_trajectory.shape[0], ns='trajectory')
    ...
    while not ve.done:
        ...
        while not vt.done:
            ...
            current_state = actual_trajectory[t]
            ...
            for n in range(plans.shape[0]):
                ...
                traj_msg.type = Marker.LINE_STRIP
                traj_msg.scale.x = 0.01
                traj_msg.scale.y = 0.01
                traj_msg.scale.z = 0.01
                # randomise colour
                traj_msg.color.b, traj_msg.color.g, traj_msg.color.r = np.random.rand(3)
                traj_msg.color.a = 1.0
                traj_msg.pose.orientation.w = 1.0
                traj_msg.id = n
                for t in range(plans_ee.shape[1]):
                    point = ros_numpy.msgify(Vector3, plans_ee[n, t, :3])
            ...
            goal_msg.points = [ros_numpy.msgify(Vector3, goal)]

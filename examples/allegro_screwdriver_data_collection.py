...
def do_trial(env, params, fpath, sim_viz_env=None, ros_copy_node=None):
    ...
    # We add some noise to the default dof pos for diversity in how the contact is made
    default_dof_pos = 0.25 * torch.randn_like(env.default_dof_pos[:, :16]) + env.default_dof_pos[:, :16]
    default_dof_pos_pregrasp = 0.25 * torch.randn_like(env.default_dof_pos[:, :16]) + env.default_dof_pos[:, :16]
    ...
    if params['controller'] == 'csvgd':
        ...
        pregrasp_problem = AllegroScrewdriver(
            ...
            default_dof_pos=default_dof_pos_pregrasp
        )
        # finger gate index
    ...
    def execute_traj(planner, goal=None, fname=None):
        ...
        for k in range(planner.problem.T):  # range(params['num_steps']):
            ...
            if params['visualize'] and best_traj.shape[0] > 1:
                add_trajectories(plans.to(device=params['sim_device']), best_traj.to(device=params['sim_device']),
    ...
    # Randomly sample contact type
    contact_class = np.random.choice(['turn', 'regrasp_index',
                                      'regrasp_thumb', 'regrasp_middle',
                                      'regrasp_thumb_and_middle'])

    # plan to contact
    traj, plans, contact_points, contact_distance = execute_traj(
        pregrasp_planner, fname=f'pregrasp')
    ...
    if contact_class == 'turn':
        traj, plans, contact_points, contact_distance = execute_traj(
            turn_planner, goal=valve_goal, fname=f'turn')
        _add_to_dataset(traj, plans, contact_points, contact_distance, contact_state=torch.ones(3))
    elif contact_class == 'regrasp_index':
        traj, plans, contact_points, contact_distance = execute_traj(
            index_regrasp_planner, goal=valve_goal, fname=f'index_regrasp')
    ...
    elif contact_class == 'regrasp_middle':
        traj, plans, contact_points, contact_distance = execute_traj(
            middle_regrasp_planner, goal=valve_goal, fname=f'middle_regrasp')
        ...
        traj = torch.cat((traj[..., :-3], torch.zeros(*traj.shape[:-1], 3).to(device=params['device']),
                          traj[..., -3:]), dim=-1)
        _add_to_dataset(traj, plans, contact_points, contact_distance, contact_state=torch.tensor([1.0, 0.0, 1.0]))

    elif contact_class == 'regrasp_thumb':
        traj, plans, contact_points, contact_distance = execute_traj(
            thumb_regrasp_planner, goal=valve_goal, fname=f'thumb_regrasp')
        plans = [torch.cat((plan,
                            torch.zeros(*plan.shape[:-1], 3).to(device=params['device'])),
                           dim=-1) for plan in plans]
        traj = torch.cat((traj, torch.zeros(*traj.shape[:-1], 3).to(device=params['device'])), dim=-1)
        _add_to_dataset(traj, plans, contact_points, contact_distance, contact_state=torch.tensor([1.0, 1.0, 0.0]))
    elif contact_class == 'regrasp_thumb_and_middle':
        traj, plans, contact_points, contact_distance = execute_traj(
            thumb_and_middle_regrasp_planner, goal=valve_goal, fname=f'thumb_middle_regrasp')
        plans = [torch.cat((plan,

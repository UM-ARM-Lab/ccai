...
def do_trial(env, params, fpath, sim_viz_env=None, ros_copy_node=None, inits_noise=None, noise_noise=None, sim=None, seed=None,
    ...
    def execute_traj(planner, mode, goal=None, fname=None, initial_samples=None, recover=False, 
        ...
        if 'mppi' in params['recovery_controller'] and recover:
            ...
            if mode == 'index' and planner is None:
            ...
            elif mode == 'index_thumb' and planner is None:
                index_thumb_regrasp_problem = AllegroValve(
                    start=state[:4 * num_fingers + obj_dof],
                    goal=goal,
                    T=params['T'],
                    chain=params['chain'],
                ...
                planner = PositionControlConstrainedSVGDMPC(index_thumb_regrasp_problem, recovery_params)
                
            elif mode == 'index_middle' and planner is None:
                index_middle_regrasp_problem = AllegroValve(
                    start=state[:4 * num_fingers + obj_dof],
                    goal=goal,
                    T=params['T'],
                    chain=params['chain'],
        ...
        if (params['visualize_plan'] and not recover) or (params['visualize_recovery_plan'] and recover):
            k = -1
            traj_for_viz = initial_samples[0, :, :planner.problem.dx]
            if params['exclude_index']:
                traj_for_viz = torch.cat((state[4:4 + planner.problem.dx].unsqueeze(0), traj_for_viz), dim=0)
            else:
                traj_for_viz = torch.cat((state[:planner.problem.dx].unsqueeze(0), traj_for_viz), dim=0)
        ...
        for k in range(start_timestep, total_steps):
            ...
            if params['mode'] == 'hardware':
                ...
                set_state = set_state.reshape(-1, 4 * num_fingers + obj_dof)[0].to(device=params['sim_device'])
    ...
    def plan_recovery_contacts_w_model(state):
        ...
        best_mode_traj, _, best_mode_likelihoods = trajectory_sampler.sample(N=contact_mode_pred.shape[0], start=start_for_diff.reshape(1, -1),
    ...
    while episode_num_steps < max_episode_num_steps and not safety_break:
        ...
        if stage == 0:
            ...
            contact = 'pregrasp'
            if 'pregrasp_state' not in params:
                ...
                # Step to default_dof_pos
                env.step(default_dof_pos_action)
                time.sleep(1)
                env.step(params['pregrasp_state'].reshape(1, -1)[:, :4 * num_fingers])
            # for _ in range(50):
            #     env._step_sim()

            if params['mode'] == 'hardware':
                print('Desired valve angle:', params['pregrasp_state'][-1])
            ...
            goal_yaw = post_pregrasp_state[-1] - np.pi / 3
        ...
        if contact == 'middle':
            ...
            if mppi_ctrl is None:
                mppi_ctrl = MPPI(dynamics=dynamics, running_cost=running_cost, terminal_state_cost=terminal_cost, nx=4*num_fingers + obj_dof, noise_sigma=noise_sigma, 
...
if __name__ == "__main__":
    ...
    # config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/examples/config/valve/allegro_valve_csvto_recovery_model_hardware.yaml').read_text())
    # config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/examples/config/valve/allegro_valve_csvto_no_recovery_hardware.yaml').read_text())
    config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/examples/config/valve/allegro_valve_mppi_likelihood_recovery_hardware.yaml').read_text())
    # config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/examples/config/valve/allegro_valve_mppi_safe_rl_recovery_hardware.yaml').read_text())
    ...
    if config['mode'] == 'hardware':
        ...
        default_dof_pos = torch.cat((torch.tensor([[0.2, 0.5, 0.65, 0.8]]).float(),
                                          ...
                                          dim=1)
        default_dof_pos_action = torch.cat((torch.tensor([[0.2, 0.5, 0.65, 0.8]]).float(),
                                          torch.tensor([[-0.1, 0.2, 0.9, 0.8]]).float(),
                                          torch.tensor([[1.2, 0.2, 0., 1.0]]).float()),
                                          dim=1).reshape(1, -1)
        env = HardwareEnv(default_dof_pos[:, :16], 
                          ...
                          num_repeat=10)
        env.obj_pose = np.array([0.0, 0.0, 0.40])
        env.get_state()
        for _ in range(5):
        ...
        sim_env = RosAllegroValveTurningEnv(num_envs, control_mode='joint_impedance',
    ...
    if model_path is not None:
        ...
        def load_sampler(path, dim_mults=(1,2), T=config['T'], recovery=False):
            ...
            # if not recovery:
            #     trajectory_sampler.warmup_completed = False
            #     trajectory_sampler.warmup_model(warmup_batch_size=8, warmup_horizon=None)
        ...
        if model_path_orig is not None:
            ...
            if not config.get('generate_context', False):
                ...
                model_path_classifier = config.get('model_path_classifier', None)
                classifier_d = torch.load(f'{CCAI_PATH}/{model_path_classifier}', map_location=torch.device(params['device']))
        ...
        if config['recovery_controller'] == 'mppi':
            ...
            dynamics = DynamicsModel(env_for_mppi, num_fingers=len(config['fingers']), include_velocity=True, obj_joint_dim=0, hardware=False)
    ...
    with open(f'{CCAI_PATH}/examples/pregrasp_states_valve.pkl', 'rb') as f:
    ...
    # for i in tqdm([1, 2, 4, 7]):
        ...
        object_location = torch.from_numpy(np.array([0.0, 0.0, 0.40])).to(device=params['device']).float()  # TODO: confirm if this is the correct location
        params['object_location'] = object_location

        # If params['device'] is cuda:1 but the computer only has 1 gpu, change to cuda:0
        succ = False
        while not succ:
            params['pregrasp_state'] = pregrasp_states[i]

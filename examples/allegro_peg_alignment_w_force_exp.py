...
class AllegroPegAlignment(AllegroValveTurning):
    ...
    def __init__(self,
        ...
        self.min_force_dict = {'index':2.0, 'middle':2.0, 'ring': 0.5, 'thumb': 3.0}
    ...
    def _thumb_repulsive(self, xu, link_name, compute_grads=True, compute_hess=False):
        ...
        g = -ret_scene.get('sdf').reshape(N, T + 1, 1)  + 0.003
        grad_g_q = -ret_scene.get('grad_sdf', None)
    ...
    def _cost(self, xu, start, goal):
        ...
        # uprightness cost
        smoothness_cost += 1000 * torch.sum((state[:, -2:]) ** 2)

        action_cost = 50 * torch.sum((next_q - state[1:, :-self.obj_dof]) ** 2)
        goal_cost = 0
        if self.obj_translational_dim:
            ...
            smoothness_cost = smoothness_cost + 1e6 * torch.sum((obj_position[1:] - obj_position[:-1]) ** 2)
        if self.obj_rotational_dim:
            ...
            goal_cost = goal_cost + torch.sum((5000 * (obj_orientation[-1] - goal_orientation) ** 2))
            # running cost 
            goal_cost = goal_cost + torch.sum((20.0 * (obj_orientation - goal_orientation) ** 2))
            smoothness_cost = smoothness_cost + 500 * torch.sum((obj_orientation[1:] - obj_orientation[:-1]) ** 2)
...
def do_trial(env, params, fpath, sim_viz_env=None, ros_copy_node=None):
    ...
    start = env.get_state()['q'].reshape(robot_dof + obj_dof).to(device=params['device'])
    ...
    if params['visualize_plan']:
        ...
        visualize_trajectory(traj_for_viz, pregrasp_problem.contact_scenes, viz_fpath, 
                             ...
                             use_arm=params['use_arm'], camera_params=camera_params)
    ...
    for k in range(params['num_steps']):
        ...
        if params['visualize_plan']:
            ...
            pathlib.Path.mkdir(img_fpath, parents=True, exist_ok=True)
            pathlib.Path.mkdir(gif_fpath, parents=True, exist_ok=True)
            visualize_trajectory(traj_for_viz, turn_problem.robot_peg_scenes, viz_fpath, turn_problem.fingers, 
                                 turn_problem.obj_dof, use_arm=params['use_arm'], camera_params=camera_params)
            
            img_fpath = pathlib.PurePath.joinpath(viz_fpath, 'img_obj')
            gif_fpath = pathlib.PurePath.joinpath(viz_fpath, 'gif_obj')
            pathlib.Path.mkdir(img_fpath, parents=True, exist_ok=True)
            pathlib.Path.mkdir(gif_fpath, parents=True, exist_ok=True)
            visualize_obj_trajectory(traj_for_viz[:, robot_dof: robot_dof + obj_dof], turn_problem.peg_wall_scenes, viz_fpath)
        ...
        if params['optimize_force']:
            print("planned force")
            print(action[:, robot_dof:].reshape(num_fingers_to_plan + 1, 3)) # print out the action for debugging

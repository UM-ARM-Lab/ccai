...
def do_trial(env, params, fpath, sim_viz_env=None, ros_copy_node=None, inits_noise=None, noise_noise=None, sim=None, seed=None,
    ...
    def execute_traj(planner, mode, goal=None, fname=None, initial_samples=None, recover=False, 
        ...
        if not skip_diff_init and (trajectory_sampler is not None or trajectory_sampler_orig is not None) and params.get('diff_init', True) and initial_samples is None and (not params.get('task_model_path', None) or not recover) and (not (mode != 'turn' and not params.get('live_recovery'))):

...
def index_jacobian_impl(q: torch.Tensor, link_name: str) -> torch.Tensor:
    """
    Jacobian calculation for index finger.
    ...
    if link_name == 'allegro_hand_hitosashi_finger_finger_link_1':
        J[:, 0, 0] = 2.16840434497101e-18
        J[:, 4, 0] = 0.0871557427476582
        J[:, 5, 0] = 0.996194698091746
    elif link_name == 'allegro_hand_hitosashi_finger_finger_link_2':
    ...
    elif link_name == 'allegro_hand_hitosashi_finger_finger_link_3':
    ...
    elif link_name == 'allegro_hand_hitosashi_finger_finger_0_aftc_base_link':
        ...
        raise ValueError(f"Unknown link name {link_name} for index finger")

    # Return the Jacobian matrix
    return J

@lru_cache(maxsize=128)
def index_jacobian_single(q_tuple: Tuple[float, ...], link_name: str) -> torch.Tensor:
    """Cached Jacobian implementation for single joint configuration."""
    q = torch.tensor([q_tuple], dtype=torch.float32)
    return index_jacobian_impl(q, link_name)[0]

def index_jacobian(q: torch.Tensor, link_name: Optional[str] = None, device: Optional[torch.device] = None) -> torch.Tensor:
    """
    Jacobian calculation for the index finger with PyTorch.
    ...
    if q.dim() == 1:
        ...
        return index_jacobian_single(q_tuple, link_name).to(device)

    if q.dim() != 2:
        q = q.reshape(-1, 4)

    # Call implementation function
    J = index_jacobian_impl(q, link_name)

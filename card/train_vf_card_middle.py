...
def stack_trajs(trajs):
    ...
    poses = stacked.reshape(-1, 22)
    poses = convert_full_to_partial_config(poses)
    return poses

def save_train_test_splits(validation_proportion=0.1, seed=None):

    filename = f'card_datasets/combined_middle_dataset.pkl'
    ...
    with open(f'{fpath.resolve()}/{filename}', 'rb') as file:
        ...
        trajs1, costs, start_ys = zip(*tuples)
    ...
    path = f'{fpath.resolve()}/value_functions/middle_dataloader.pkl'
...
def load_ensemble(device='cpu', model_name = "throwerror"):
    ...
    checkpoints = torch.load(f'{fpath.resolve()}/value_functions/{model_name}.pkl')
...
if __name__ == "__main__":
    ...
    path = f'{fpath.resolve()}/value_functions/middle_vf.pkl'
    model_name = f"middle_vf"
    ensemble = []
    for i in range(16):

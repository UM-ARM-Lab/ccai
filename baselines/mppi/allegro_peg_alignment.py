...
class ValidityCheck:
    ...
    def check_validity(self, state):
        peg_state = state[0, -self.obj_dof:]
        validity_flag = check_peg_validity(peg_state)
...
class RunningCost:
    ...
    def __call__(self, state, action):
        ...
        # dropping cost
        dropping_cost = torch.sum(1e4 * (((obj_position[:, 2] < 0) * obj_position[:, 2])) ** 2)
...
if __name__ == "__main__":
    ...
    env = AllegroPegAlignmentEnv(config['controllers']['mppi']['N'], control_mode='joint_impedance',

...
class JaxCSVTOParams:
    ...
    def __init__(self, N: int, T: int, K: int, anneal: bool, alpha_J: jnp.float32, alpha_C: jnp.float32,
                 step_scale: jnp.float32, penalty_weight: jnp.float32) -> None:
        """Set the parameters for the optimization problem.

        Args:
            N: The number of trajectories to represent the distribution with.
            T: The number time horizon, which is the number of discrete states in each trajectory.
...
class JaxCSVTOpt:
    ...
    def solve(self, x0: jnp.array) -> jnp.array:
        ...
        Returns:
        """
        # Initialize slack variables and load augmented state.

        # Initialize the annealing weight gamma if annealing is on.

        # Loop over the gradient step iterations.

        # Compute the gradients and hessians for the problem functions.

        # Compute the constraint step.

        # Compute the tangent step, which accounts for annealing configuration internally.

        # Compute the combined step.

        # Clamp the step in bounds.
    ...
    def _compute_gradients(self, xuz: jnp.array) -> Tuple[jnp.array]:
        """
    ...
    def _compute_constraint_step(self, xuz: jnp.array) -> jnp.array:
        """
    ...
    def _compute_tangent_step(self, xuz: jnp.array) -> jnp.array:
        """

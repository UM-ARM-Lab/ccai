...
class AllegroScrewDriverDataset(Dataset):
    ...
    def __init__(self, folders, max_T, cosine_sine=False, states_only=False, 
        ...
        pre_num_traj = self.trajectories.shape[0]
        # Filter dropped trajectories
        roll = self.trajectories[:, -1, 12].abs()
        pitch = self.trajectories[:, -1, 13].abs()
        dropped = (roll > .35) | (pitch > .35)
        ...
        post_drop_num_traj = self.trajectories.shape[0]

        print(f'Filtered {pre_num_traj - post_drop_num_traj} dropped trajectories')

        # Filter bad turning trajectories
        start_yaw = self.trajectories[:, 0, 14]
        end_yaw = self.trajectories[:, -1, 14]
        yaw_diff = end_yaw - start_yaw
        bad_turn = (yaw_diff > -.5) & (self.trajectory_type.sum(1) == 3)
        ...
        post_turn_filter_num_traj = self.trajectories.shape[0]
        print(f'Filtered {post_drop_num_traj - post_turn_filter_num_traj} bad turning trajectories')

...
class DummyEnv:
    def __init__(self, observation_space, action_space):
        self.observation_space = torch.zeros(observation_space)
        self.action_space = torch.zeros(action_space)
...
class Experiment:
    ...
    def experiment_setup(self):
        ...
        # self.env.reset(seed=self.exp_cfg.seed)
        # self.env.action_space.seed(self.exp_cfg.seed)
        agent = self.agent_setup(self.env)
        self.agent = agent
        # if self.exp_cfg.use_recovery and not (
        #         self.exp_cfg.MF_recovery
        #         or self.exp_cfg.Q_sampling_recovery):
    ...
    def get_offline_data(self):
        ...
        if not self.exp_cfg.task_demos:
            if self.exp_cfg.env_name == 'reacher':
                ...
                constraint_demo_data = dataset.return_as_numpy()
            else:
                # Navigation 1 and 2
    ...
    def run(self):
        ...
        if not self.exp_cfg.disable_offline_updates and (
            ...
            # Save the recovery critic
            pathlib.Path.mkdir(pathlib.Path(self.logdir), parents=True, exist_ok=True)
            torch.save(
                self.agent.safety_critic.safety_critic.state_dict(), f'{self.logdir}/safety_critic.pt')

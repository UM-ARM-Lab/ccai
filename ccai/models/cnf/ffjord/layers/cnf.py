...
class CNF(nn.Module):
    ...
    def forward(self, z, logpx=None, context=None, integration_times=None, reverse=False):

        _logpx = torch.zeros(z.shape[0], 1).to(z)

        if context is None:
            state = (z, _logpx)
        else:
            state = (z, _logpx, context)
